...
export const useHeartsActions = (
  ...
  const getAllCardValueIds = () =>
    Object.values(get().cardValuesById).map((cardValue) => cardValue.id)
  return {
    INIT: {
      domain: `System`,
      run: () => {
        ...
        run(`CREATE_DECK`, {
          ...
          targets: {
            ...target(`zoneId`, `main-deck-zone`),
            cardValueIds: getAllCardValueIds(),
          },
        })
        forEach<Player>(`playersById`, (p) => {
          ...
          run(`CREATE_ZONE`, {
            ...
            targets: {
              ownerId: p.id,
              ...target(`zoneLayoutId`, zoneLayoutIdStr),
            },
          })
          run(`CREATE_PILE`, {
        ...
        run(`CREATE_CARD_CYCLE`, {
          ...
          targets: {
            ...
            3: every<Zone>(
              `zoneId`,
              (zone) => !!zone.ownerId && zone.contentType === `Pile`
            ),
          },
        })
        run(`DEAL_ALL`, {
          targets: { deckId: match(`cardGroupId`, `main-deck`) },
        })
        return {}
      },
    },
...
export const hearts = {
  name: `Hearts`,
  description: `The classic trick-taking game where points are bad...until they're not!`,
  useHeartsActions,
}
