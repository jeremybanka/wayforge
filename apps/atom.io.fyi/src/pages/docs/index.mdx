  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        selector, its `set` method is called, which should be written to result
...
Often, the data you need is not immediately available. For example, if you are fetching data from a server, you might use the fetch function `atom.io` offers natural support for `Promise` and `async/await` patterns.
