...
// const makeFilePathForJsonResource = (type: string, name: string, uni) =>
...
export const SaveJsonWebsocketServer = (
  ...
  { formatter = (s) => s, jsonRoot }: SaveJsonOptions
): WebSocketServer<
  ...
  new WebSocketServer(port, opts).on(
    ...
    (
      ...
      socket.on(`write`, ({ id, type, value }) => {
        ...
        const nextFilepath = `${type}/${filename}.json`
        ...
        if (prevFileName && prevFilePath !== nextFilepath) {
          renameSync(
            ...
            `${jsonRoot}/${nextFilepath}`
          )
        }
        writeFileSync(`${jsonRoot}/${nextFilepath}`, formatted)
      })
