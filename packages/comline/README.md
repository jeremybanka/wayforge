...
Run `pnpm --filter comline test:completions` with Bash, bash-completion, Zsh, Fish, Nushell, Carapace, Node, npm, and Bun available. Go is not required. Set `BASH_COMPLETION_FILE` if bash-completion is not at `/usr/share/bash-completion/bash_completion`. The dedicated CI job installs the consumers and runs this suite; missing tools fail rather than silently skipping coverage.
