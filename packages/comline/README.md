...
Run `pnpm exec turbo run test:completions --filter=comline` with Bash, bash-completion, Zsh, Fish, Nushell, Carapace, Node, npm, and Bun available. Turbo builds workspace dependencies before running the suite. Go is not required. Set `BASH_COMPLETION_FILE` if bash-completion is not at `/usr/share/bash-completion/bash_completion`. The dedicated CI job installs the consumers and runs this suite; missing tools fail rather than silently skipping coverage.
