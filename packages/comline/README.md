...
Shared setup, field projections, and executable fixtures live in `__tests__/fixtures/`; assertions stay in the appropriate test directory. The standard Vitest suite runs both directories. After building workspace dependencies, run `pnpm --filter comline test:once __tests__/public` or `pnpm --filter comline test:once __tests__/private` to select a directory. Future release compatibility checks can select the public directory without maintaining a filename list. See [the test guide](__tests__/README.md) when adding or splitting assertions.
