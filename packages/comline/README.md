...
Shell compatibility tests run in comline's standard Vitest suite through `pnpm test`, or `pnpm exec turbo run test:once --filter=comline`. Have Bash, bash-completion, Zsh, Fish, Nushell, Carapace, Node, npm, and Bun available; missing tools fail rather than silently skipping coverage. Turbo builds workspace dependencies first. Go is not required. Set `BASH_COMPLETION_FILE` if bash-completion is not at `/usr/share/bash-completion/bash_completion`. To run only the shell tests after building dependencies, use `pnpm --filter comline exec vitest run __tests__/completion-shells.test.ts`.

CI includes the installed consumer versions, OS, architecture, and bash-completion file checksum in `COMLINE_TEST_ENV`, which participates in comline's normal test cache key alongside source, fixtures, build configuration, and tool configuration. Local direct Vitest runs bypass Turbo's cache; use those or Turbo's `--force` option after changing locally installed consumers unless you also update `COMLINE_TEST_ENV`.
...
Run `pnpm exec turbo run test:cobra --filter=comline` separately to check against upstream Cobra. Turbo builds workspace dependencies first. This suite requires Go, Bun, Node, and Carapace, builds the small reference executable in `__tests__/fixtures/cobra` into a temporary directory, and compares protocol responses for descriptions, inline values, spacing, and filesystem directives. It also reproduces the Carapace Cobra-bridge limitation. A separate step in the Vitest CI job runs these checks. Go produces only the temporary test executable; it contributes no shipped code.
