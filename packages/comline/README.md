...
Warnings do not independently fail parsing. Existing route and validation errors still throw; warnings are returned only on successful invocations. Public interpretation and completion contexts defer warnings until the route is complete and error-free, since unfinished input can still consume options through descendant routes. An unknown option followed by a separate value may still cause a positional-argument error because its value consumption is unknown.
