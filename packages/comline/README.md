...
Shared setup, field projections, and executable fixtures live in `__tests__/fixtures/`; assertions stay in the appropriate test directory. The standard Vitest suite runs both directories. After building workspace dependencies, run `pnpm --filter comline exec vitest run __tests__/public` or `pnpm --filter comline exec vitest run __tests__/private` to select a directory. Future release compatibility checks can select the public directory without maintaining a filename list. See [the test guide](__tests__/README.md) when adding or splitting assertions.
...
Shell and upstream Cobra compatibility tests run in comline's standard Vitest suite through `pnpm test`, or `pnpm exec turbo run test:once --filter=comline`. Run `mise install` to provision the pinned bash-completion, Zsh, Fish, Nushell, Carapace, Node, npm, Bun, and Go versions; Bash must also be available. Mise installs Zsh through its Conda backend, including its completion functions and runtime libraries, without requiring a separate Conda installation. With mise activated, or through `mise exec --`, the repository configuration sets `BASH_COMPLETION_FILE` to the pinned upstream release, which CI also uses. If a launcher clears the environment, activate mise inside that launcher; for example, run `pnpm exec mise exec -- nr test --run` from the comline directory in Scrubs. Set that variable explicitly when testing another bash-completion installation. Missing tools fail rather than silently skipping coverage. Turbo builds workspace dependencies first. To run only the shell tests after building dependencies, use `pnpm --filter comline exec vitest run __tests__/public/completion-shells.test.ts`; that subset does not require Go.
...
The Cobra tests build the small reference executable in `__tests__/fixtures/cobra` into a temporary directory and compare its protocol responses with the Node fixture for descriptions, inline values, spacing, and filesystem directives. Go produces only the temporary test executable; it contributes no shipped code. An empty Go module cache requires downloading the pinned dependencies. To run only these tests after building workspace dependencies, use `pnpm --filter comline exec vitest run __tests__/public/completion-cobra.test.ts`.

To investigate the Carapace Cobra-bridge limitation, opt into its compatibility probe with `COMLINE_PROBE_CARAPACE_COBRA=1 pnpm --filter comline exec vitest run __tests__/private/completion-cobra.test.ts`. This additionally requires Carapace and builds a compiled fixture; the normal Cobra protocol suite does neither. The probe expects the documented upstream limitation and may fail when a consumer upgrade fixes it.
