...
Run `pnpm exec turbo run test:semver --filter=comline` from a clean checkout to check compatibility with the latest `comline@` release tag. Turbo builds the workspace break-check CLI and its dependencies first; the Break Check CI job runs this task too. Compatibility checks are uncached because the latest remote release tag can change independently of the checkout. The check needs access to the Git remote named `origin`.
