...
Tests live in `__tests__/public/` for consumer-visible behavior and `__tests__/private/` for implementation details and test infrastructure. Shared helpers and executable fixtures live in `__tests__/fixtures/`. The standard Vitest suite runs both directories. After building workspace dependencies, run `pnpm --filter comline exec vitest run __tests__/public` or `pnpm --filter comline exec vitest run __tests__/private` to select a directory. Keep public contract tests in the public directory so future release compatibility checks can select them with a directory glob.
...
Shell and upstream Cobra compatibility tests run in comline's standard Vitest suite through `pnpm test`, or `pnpm exec turbo run test:once --filter=comline`. Have Bash, bash-completion, Zsh, Fish 4+, Nushell, Carapace, Node, npm, Bun, and Go available; missing tools fail rather than silently skipping coverage. Turbo builds workspace dependencies first. Set `BASH_COMPLETION_FILE` if bash-completion is not at `/usr/share/bash-completion/bash_completion`. To run only the shell tests after building dependencies, use `pnpm --filter comline exec vitest run __tests__/public/completion-shells.test.ts`; that subset does not require Go.
...
To investigate the Carapace Cobra-bridge limitation, opt into its compatibility probe with `COMLINE_PROBE_CARAPACE_COBRA=1 pnpm --filter comline exec vitest run __tests__/public/completion-cobra.test.ts`. This additionally requires Carapace and builds a compiled fixture; the normal Cobra protocol suite does neither. The probe expects the documented upstream limitation and may fail when a consumer upgrade fixes it.
