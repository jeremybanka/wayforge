...
To investigate the Carapace Cobra-bridge limitation, opt into its compatibility probe with `COMLINE_PROBE_CARAPACE_COBRA=1 pnpm --filter comline exec vitest run __tests__/completion-cobra.test.ts`. This additionally requires Carapace and builds a compiled fixture; the normal Cobra protocol suite does neither. The probe expects the documented upstream limitation and may fail when a consumer upgrade fixes it.
