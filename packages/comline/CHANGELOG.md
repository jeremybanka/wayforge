...
- df27495: Configuration discovery is now opt-in: CLIs without `discoverConfigPath` no longer read `<cli-name>.config.json`. To retain that behavior, set `discoverConfigPath: () => path.join(process.cwd(), "<cli-name>.config.json")`.
