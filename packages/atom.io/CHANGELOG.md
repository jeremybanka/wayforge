...
- c292fcc: 📝 Expanded the getting-started docs for evaluating `atom.io` in existing React apps.

  The guide now walks through porting common React state patterns to `atom.io`, including `useState`, context state, Zustand stores, React Query, `useEffect` synchronization, timers, and transaction-based event handlers.
...
- 0d983bd: Ship generated agent-friendly documentation with `atom.io`.
...
- 228d4bf: Drop the direct `tmp` test dependency in favor of Node platform temporary directory APIs.
