...
const queryByTestId = (
	host: HTMLElement,
	testId: string,
): HTMLElement | null => host.querySelector<HTMLElement>(`[data-testid="${testId}"]`)
...
const makeButtonRef = <TT>(initialValue: TT | null): {
...
const mountWithProvider = async (
	renderChildren: (ctx: Awaited<ReturnType<typeof loadSolidIntegration>> & {
		host: HTMLElement
	}) => void,
) => {
		...
		const mounted = await mountWithProvider(({ AS, Solid, host }) => {
			...
			const Letter = () => {
				...
				root.append(
					...
					atDisplay,
					lengthDisplay,
					createButton(`changeStateButtonB`, () => setLetter(`B`)),
					createButton(`changeStateButtonC`, () => setLetter(`C`)),
					createButton(`undoButton`, () => letterTimeline().undo()),
					createButton(`redoButton`, () => letterTimeline().redo()),
				)
				Solid.createEffect(() => {
					const value = letter()
		...
		const mounted = await mountWithProvider(({ AS, Solid, host }) => {
			const Letter = () => {
				...
				root.append(
					...
					atDisplay,
					lengthDisplay,
					createButton(`changeLetterButtonB`, () => setLetter(`B`)),
					createButton(`changeNumberButton2`, () => setNumber(2)),
				...
				Solid.onCleanup(() => disposeTimeline())
				return root
			}
		...
		const indexAtoms = atomFamily<Loadable<number[]>, number, Error>({
			...
			default: (key) => {
				...
				return new Promise((resolve, reject) => {
					loadIndex[key] = () => resolve([1, 2, 3])
					failIndex[key] = () => reject(new Error(`💥`))
				})
			},
			catch: [Error],
		})
		...
		const { dispose, host } = await mountWithProvider(({ AS, Solid, host }) => {
			const Letter = () => {
				...
				Solid.createEffect(() => {
					...
					if (value.error) {
						setDisplay(error, `error`, `${(value.error as Error).message}`)
						children.push(error)
					}
		...
		const letterAtom = atom<Loadable<string>, Error>({
			...
			default: () =>
				new Promise<string>((resolve) => {
					loaders.push((letter: string) => resolve(letter))
				}),
			catch: [Error],
		})
		const { dispose, host } = await mountWithProvider(({ AS, Solid, host }) => {
