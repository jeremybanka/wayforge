...
describe(`Filebox`, () => {
	test(`mode:off`, async () => {
		...
		const result = await responses.get(`http://localhost:12500`)
	...
	test(`mode:read`, async () => {
		...
		try {
			await get(`http://localhost:12500`)
		} catch (thrown) {
		...
		fs.writeFileSync(
			path.join(tempDir.name, `hello.input.json`),
			`["http://localhost:12500"]`,
		)
		fs.writeFileSync(
			path.join(tempDir.name, `hello.output.json`),
			`"The best way to predict the future is to invent it."`,
		)

		const result = await get(`http://localhost:12500`)
		expect(result).toBe(`The best way to predict the future is to invent it.`)
		expect(utils.put).toHaveBeenCalledTimes(0)
	})
})
