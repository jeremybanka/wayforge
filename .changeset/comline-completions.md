...
Add opt-in tab completion for Bash, Zsh, Fish, Nushell, and Carapace, driven by the same CLI definitions and argument interpretation as normal parsing. Integrations work with globally installed commands and Bun-compiled executables. Shell adapters use the engine's replacement boundaries to preserve literal positional arguments and option values, encode Nushell literals without expansion, quote shell syntax, and retain directory continuation. Suggest reachable options before command selection. Ordinary parsing avoids completion presentation work, and completion shares the scanner's pending-value state. Route-specific consumption prevents unreachable sibling options from swallowing command words; genuinely ambiguous boundaries require an inline value or `--`.
...
Use `completionResponse(definition, process.argv)` at the CLI entry point to opt in to completion requests and installation commands. `completion <target>` prints a standalone integration file; `completionScript()` provides the same generation API. Support Cobra's `__complete` and `__completeNoDesc` protocols and Carapace's native JSON export protocol. Fish preserves the exact candidate set and follows its native spacing rules; it does not support arbitrary no-space hints.
