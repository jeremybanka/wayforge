...
Export `TreePathParams` to infer named capture values from each alternative path name. `Join` supports variadic paths, including their shortest valid form. Path validation considers every matching branch and checks the supplied path without imposing capture-naming rules on the tree.
